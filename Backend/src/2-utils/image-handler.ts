...
const vacationImageFolder = "./src/1-assets/images/";
...
async function saveImage(image: UploadedFile): Promise<string> {
    ...
    const absolutePath = vacationImageFolder + uniqueImageName;
...
async function deleteImage(existingImageName: string): Promise<void> {
    try {
        ...
        await fsPromises.unlink(vacationImageFolder + existingImageName);
    }
    catch(err: any) {
...
function getAbsolutePath(imageName: string): string {
    ...
    if(!fs.existsSync(absolutePath)) {
        absolutePath = path.join(__dirname, "..", "1-assets", "images", "not-found.webp");
    }
    return absolutePath;
}
